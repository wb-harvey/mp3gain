...
#include <stdio.h>
struct MP3GainTagInfo *globalTagInfoHack = NULL;
#include <stdlib.h>
#include <math.h>
#include "apetag.h"
#include "id3tag.h"
...
#ifdef WIN32
int __cdecl mp3gain_main(int argc, char **argv) { /*make sure this one is standard C declaration*/
#else
    ...
    for (mainloop = fileStart; mainloop < argc; mainloop++) {
      ...
      if ((!skipTag)&&(!deleteTag)) {
...
#endif
		{
			...
			if (forceRecalculateTag || deleteTag || skipTag) {
				tagInfo[mainloop].haveTrackGain = 0;
				tagInfo[mainloop].haveTrackPeak = 0;
				tagInfo[mainloop].haveAlbumGain = 0;
				tagInfo[mainloop].haveAlbumPeak = 0;
...
/*MAA*/			}
	...
	if (globalTagInfoHack != NULL && argc > 0) {
		*globalTagInfoHack = tagInfo[argc - 1];
	}
    free(tagInfo);
